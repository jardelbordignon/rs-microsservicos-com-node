...
- `GET http://localhost:4002/health` deve retornar HTTP 200 com body contendo `database` e `rabbitmq` em `info`/`details`.
...
cd checkout-service && pnpm run start:dev
...
curl -s -o /dev/null -w "%{http_code}" http://localhost:4002/health
# Esperado: 503
```
