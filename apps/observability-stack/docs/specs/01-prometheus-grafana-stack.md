...
|----------------------|--------|------------------------------------|
| users-service        | 4005   | API de usuários (NestJS)           |
| products-service     | 4004   | API de produtos (NestJS)           |
| payments-service     | 4003   | API de pagamentos (NestJS)         |
| checkout-service     | 4002   | API de checkout/pedidos (NestJS)   |
| api-gateway          | 4001   | Gateway HTTP (NestJS)              |
| **Grafana**          | **4000** | **UI de dashboards (novo)**      |
| RabbitMQ (AMQP)      | 567 | Protocolo de mensageria            |
...
- **Porta do Grafana 4000→4000**: o Grafana roda internamente e é exposto na 4000.
- **Volumes dados**: `prometheus_data` e `grafana_data` persistem dados entre restarts.
...
- Grafana datasources: http://localhost:4000 → Connections → Data sources
- Health check Prometheus: http://localho9090/-/healthy
- Health check Grafana: http://localhost:4000/api/health
## Pré-requisitos
...
- Acessar `http://localhost:4000` deve exibir a tela de login do Grafana.
- Login com `admin/admin` devuncionar.
- Acessar `http://localhost:4000/api/health` deve retornar status `ok
...
- Acessar `http://localhost:9090/targets` deve listar 5 targets configurados:
  - `users-service` → `host.docker.internal:4005`
  - `products-service` → `host.docker.internal:4004`
  - `payments-service` → `host.docker.internal:4003`
  - `checkout-service` → `host.docker.internal:4002`
  - `api-gateway` → `host.docker.internal:4001`
...
- A porta 4000 (Grafana) não deve conflitar com nenhum serviço existente do marketplace.
