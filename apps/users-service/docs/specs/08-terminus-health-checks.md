...
- `GET http://localhost:4005/health` deve retornar HTTP 200 com body contendo `status`, `info`, `error` e `details`.
...
curl -s -o /dev/null -w "%{http_code}" http://localhost:4005/health
# Esperado: 503
...
| `users-service/src/domain/health/health.controller.ts` | Reescrever — usar terminus |
| `users-service/src/domain/health/health.module.ts` | Criar |
| `users-service/src/domain/domain.module.ts` | Alterar — importar `HealthModule`, remover `HealthController` do controllers |
