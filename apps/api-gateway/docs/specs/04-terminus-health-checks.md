...
|---------|-----------|---------------------|
| checkout-service | `http://localhost:4002` | `CHECKOUT_SERVICE_URL` |
| payments-service | `http://localhost:4003` | `PAYMENTS_SERVICE_URL` |
| products-service | `http://localhost:4004` | `PRODUCTS_SERVICE_URL` |
| users-service | `http://localhost:4005` | `USERS_SERVICE_URL` |
...
### 6.1 HealthController (`src/domain/health/health.controller.ts`)
...
```json
{
  ...
  "error": {
    "payments-service": {
      "status": "down",
      "message": "connect ECONNREFUSED 127.0.0.1:4003"
    }
  },
  "details": {
    "users-service": { "status": "up" },
...
### 6.2 HealthModule (`src/domain/health/health.module.ts`)
...
|---------|-------------------|
| `src/domain/health/health.service.ts` | Substituído pelo terminus |
...
- `GET http://localhost:4001/health` deve retornar HTTP 200 com body contendo os 4 serviços (`users-service`, `products-service`, `checkout-service`, `payments-service`) em `info`/`details`.
...
# 2. Verificar health check com todos os serviços UP
curl -s http://localhost:4001/health | jq .
# Esperado: { "status": "ok", "info": { "users-service": ..., "products-service": ..., ... } }

# 3. Parar um serviço downstream
kill <pid-payments-service>

# 4. Verificar health check com serviço DOWN
curl -s -o /dev/null -w "%{http_code}" http://localhost:4001/health
# Esperado: 503

curl -s http://localhost:4001/health | jq .
# Esperado: { "status": "error", ..., "error": { "payments-service": { "status": "down", ... } } }

# 5. Verificar que endpoints antigos foram removidos
curl -s -o /dev/null -w "%{http_code}" http://localhost:4001/health/services
# Esperado: 404

curl -s -o /dev/null -w "%{http_code}" http://localhost:4001/health/ready
# Esperado: 404
```
...
| `api-gateway/src/domain/health/health.controller.ts` | Reescrever — usar terminus com HttpHealthIndicator |
| `api-gateway/src/domain/health/health.module.ts` | Reescrever — usar TerminusModule + HttpModule |
| `api-gateway/src/domain/health/health.service.ts` | Remover |
