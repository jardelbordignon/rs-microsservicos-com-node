...
O teste E2E deve ser executado inteiramente via `api-gateway` (porta 4001), validando a integração de todos os serviços do marketplace de ponta a ponta.

**Pré-requisitos:** todos os serviços devem estar rodando (users-service:4005, products-service:4004, checkout-service:4002, payments-service:4003, api-gateway:4001, PostgreSQL, RabbitMQ).
...
- `GET /payments/:orderId` no `api-gateway` (porta 4001) deve encaminhar a requisição para `payments-service` (porta 4003) e retornar a resposta corretamente.
...
```
                           API Gateway (:4001)
                          ┌─────────────────────┐
                          │  AuthProxyController │──▶ users-service (:4005)
                          │  UsersProxyController│──▶ users-service (:4005)
                          │  ProductsController  │──▶ products-service (:4004)
                          │  CartProxyController │──▶ checkout-service (:4002)
                          │  OrdersProxyController──▶ checkout-service (:4002)
                          │  PaymentsProxyController──▶ payments-service (:4003)  ← NOVO
                          └─────────────────────┘

checkout-service (:4002)                    payments-service (:4003)
┌──────────────────┐                        ┌────────────────────────┐
...
- O `gateway.config.ts` já possui a configuração do `payments-service` com URL `http://localhost:4003` e timeout de `10000ms`. Não é necessário alterar este arquivo.
